...
- **待ち時間があるなら、言語の差はそこに埋もれる**(2026-09-08 更新・ROADMAP G)。同じアルゴリズムの
  TypeScript 版を書いて同じ合成サイトを巡回させた。応答 50ms のとき Go と TypeScript の差は 10% 以内で、
  **実行をまたいでも再現する**(実行間で 0.99〜1.08 倍・0.97〜1.04 倍)。
- **待ち時間の無い条件は、この機械では再現しない。** 合成サイトを Go で書き直して受け側の律速を外したうえで
  測っても、同じ条件の倍率が実行ごとに 0.96 / 1.70 / 2.09 / 3.55 と動く。純計算(URL 正規化 20 万回)も
  1.52〜3.06 倍で動く。**1 回の実行の中では安定して見えるので、実行をまたがないと分からない。**
  事前登録した予測「遅延ゼロでも差は 30% 未満」は**外れ**だった —— Go のほうが速いが、倍率は言えない
