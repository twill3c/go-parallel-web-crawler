...
| F-39 | **Go と TypeScript の比較**(L10 実装)。同じアルゴリズムの TS 実装(`bench/crawler.ts`)を書き、同じ合成サイトを同じ設定で巡回させて実測する。時間を測る前に**両実装が同じ結果を返すこと**を検査で固定する。結果は `public/bench-results.json` に条件つきで書き、画面は G-16 に従って表示する。**この数を言語の一般的な速さとして扱わない**(原本 §34 G の但し書き) | could | §34 G |
...
| G-16 | 言語比較は、**比較が成立した条件でだけ倍率を出す**。成立の条件は四つ揃うこと ——(a) 両実装が同じページ数・同じ完了理由に達した (b) 合成サイトが律速でない(Go の Workers を N にしたときの速度倍率が N×0.6 以上) (c) 中央値が両方 500ms 以上 (d) **同じ条件の最大 / 最小が 2 倍以内**(L13 で追加)。崩れた条件では倍率を出さず理由を書く | `bench/run.mjs` が判定して JSON に書き、`langRows` が表示を抑える。単体・実ブラウザ・陽性対照 4 種 | L10 / L13 |
...
| D-11 | 言語比較の結論(L13 実測・2026-09-08)。**待ち時間のある条件では Go と TypeScript の差は 10% 以内で、実行をまたいでも再現する**(50ms 遅延・実行間 0.99〜1.08 倍と 0.97〜1.04 倍)。**待ち時間の無い条件は、この機械では再現しない**(遅延 0・Go 製サイト: 実行ごとに 0.96 / 1.70 / 2.07 / 3.55 倍)。純計算(URL 正規化)も 1.52〜3.06 倍で動く。事前登録した予測 P2(遅延 0 でも差は 30% 未満)は**外れ** —— 遅延 0 では Go のほうが速いが、倍率は言えない | 「言えない」ことを言えるようにするのが `crossRun`。実行ごとの要約を `bench/history.jsonl` に追記し、同じ条件の比の範囲を出す |
...
| D-10 | 遅延ゼロの条件では**合成サイトを Go で書く**(`cmd/benchsite`)。Node 版は 1 スレッドなので受け側が先に飽和し、クローラでなくサーバを測ることになる(2026-09-07 実測: Go の Workers 5 が 1 に対して 2.7 倍しか出なかった)。同じ条件を Node 製サイトでも回して**対照**として残す —— サーバの実装が結論を変えることを見せるため | 交絡を「避ける」のでなく「両方測って見せる」ほうが、読み手が判断できる |
| D-08 | 言語比較は**手元で測って結果を配る**(本番で走らせない)。本番は Go の実行環境しか無く、Node を並べて置けない。測定器は `bench/` に残し、誰でも同じ手順で測り直せるようにする | 本番で両方を走らせる方法が無い以上、「測れないものを測ったふりをする」より「測った条件を明記して配る」ほうがよい |
| D-09 | TS 版の HTML 解析は Go 版と**別実装**(x/net/html のトークナイザ 対 自前の走査)。揃えていないことを明記し、解析時間が全体の何割かを測って添える | 同等の解析器を JS で用意すると、比べているのが「私の JS 解析器 対 Go の標準ライブラリ」になり、かえって読み違えやすい。**割合が小さいことを示して、この差が結論を汚さないと言えるようにする**(実測: TS 側で 0.6%・2.0%) |
