...
### L10(Go vs TypeScript)

**時間を測る前に、両実装が同じ結果を返すことを固定する。** 揃っていない実装の速さを比べても意味が無い。

| ID | 対応要求 | ケース | 期待 |
|---|---|---|---|
| T-1001 | F-39 | 同じ合成サイトを Go 版と TS 版で(Workers 1/5・上限あり/なし) | ページ数・完了理由が一致。上限に当たらない条件では辺の数も一致 |
| T-1002 | F-39 | 正規化・同一ドメイン・拡張子判定 | **Go 側と同じ表**(片方だけ直すと落ちる)。正規化は冪等 |
| T-1003 | F-39 | 合成サイトの HTML からの抽出 | 外部リンクも画像も抽出はする(除外は呼び手)・重複は畳む |
| T-1010 | G-16 | `langRows` / `langVerdict` | 成立した行だけが倍率を持つ。成立しない行は理由を持つ。**陽性対照**: 交絡を 4 種類(結果の食い違い・サイト律速・短すぎる計測・ばらつき)作ると倍率が消える |
| T-1011 | G-16, G-10 | 実ブラウザ: 比較の節 | 表が出る・成立しない行は「—」と理由・注意書きと測定条件が必ず出る |

### L11(サイトマップと外部ドメイン)

**後退経路を塞いでから測る**(HC-206)。robots の `Sitemap:` 行を検査するケースでは `/sitemap.xml` を
404 にし、塞がっていることをテスト内で assert してから本題に入る。

| ID | 対応要求 | ケース | 期待 |
|---|---|---|---|
| T-1101 | F-19 | `urlset` の `<loc>` を取り出す | 空白を落とす・重複を畳む・http/https 以外と壊れたものを捨てる |
| T-1102 | F-19 | `sitemapindex` | `IsIndex` が真、入れ子の場所を返す |
| T-1103 | F-19 | 壊れた XML・空・HTML・制御バイト | 落ちず、空を返す(陰性対照) |
| T-1104 | F-19 | 1,000 件を超えるサイトマップ | `SitemapMaxURLs` で打ち切り、`Truncated` が真 |
| T-1105 | F-20b | 外部集計 | ホストは `www.` を畳む・URL は正規化して数える・scheme 違いは別 URL・件数の降順 |
| T-1111 | F-19 | サイトマップが種になる | 隠れたページに到達。**対照**: 使わない設定では到達しない |
| T-1112 | F-19, G-17 | robots の `Sitemap:` 行 | **`/sitemap.xml` が 404 であることを先に確かめてから**、robots の行だけで到達できること |
| T-1113 | F-19 | サイトマップが無い / 宣言はあるが取れない | `absent` / `declared_missing`。どちらもクロールは続く |
| T-1114 | F-20b, G-17 | 外部ドメインの集計 | **辿っていない**かつ**数えている**。サブドメインは畳まない・降順 |
| T-1115 | F-20b, G-10 | 実ブラウザ: 外部ドメインの表 | 初期は隠れている・件数の降順・ページ一覧に外部ドメインが無い |
| T-1116 | F-19, G-10 | 実ブラウザ: サイトマップの切り替え | 使わないと隠れたページに届かず、使うと届く(同じ画面で両方を測る) |
